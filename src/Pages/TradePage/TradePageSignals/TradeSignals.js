    ...
    const columns = useMemo(
        () => [
            {Header: "Portfolio", accessor: "portfolio_code"},
            {
                ...
                accessor: "status",
                Cell: ({value}) => {
                    const className = value?.toLowerCase().replace(/\s+/g, "-"); // pl. "PARTIALLY EXECUTED" -> "partially-executed"
                ...
                <table {...getTableProps()} className="signals-table">
                    ...
                    {rows.map((row) => {
                        ...
                        return (
                          ...
                          {row.isExpanded ? <BsCaretUpFill/> :
                              <BsCaretDownFill/>} {cell.render("Cell")} ({row.subRows.length})
                        </span>
                                        ) : (
                                            cell.render("Cell")
                                        )}
